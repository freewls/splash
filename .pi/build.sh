...
# docker buildx build --tag splash --file .pi/Dockerfile --platform linux/arm64 --cache-from splash:cache --cache-to splash:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar splash
...
sudo bash -c "echo splash > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/splash.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/splash.img
